...
class ClipOp : public framework::OperatorWithKernel {
 public:
  ...
  void InferShape(framework::InferShapeContext* ctx) const override {
    PADDLE_ENFORCE_EQ(ctx->HasInput("X"), true, platform::errors::InvalidArgument(
                          "Input(X) of ClipOp should not be null. Please "
                          "check if it is created correctly."));
    PADDLE_ENFORCE_EQ(ctx->HasOutput("Out"), true, platform::errors::InvalidArgument(
    ...
    PADDLE_ENFORCE_LT(min, max, platform::errors::InvalidArgument(
                          "Input(min) of ClipOp is greater or equal than Input(max) of ClipOp. "
...
class ClipOpGrad : public framework::OperatorWithKernel {
 public:
  ...
  void InferShape(framework::InferShapeContext* ctx) const override {
    ...
    OP_INOUT_CHECK(ctx->HasInput(framework::GradVarName("Out")), "Input", "Out", "Clip_grad");
    auto x_dims = ctx->GetInputDim("X");
